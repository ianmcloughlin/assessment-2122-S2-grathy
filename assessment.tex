...
\begin{document}
  ...
  \subsubsection*{Thompson's Construction Notebook (40\%)}
  Include in your repository a Jupyter notebook called \texttt{finite-state.ipynb} that contains the following.
  \begin{itemize}
    \item Overview and explanation of the connection between Finite State Automata and Regular Expressions.
    \item Python function implementing Thompson's Construction~\cite{thompsonsconstruction}, taking as input a regular expression string over the alphabet \texttt {\{0,1\}} with special characters \texttt{.} for concatenation, \texttt{|} for or, and \texttt{*} for Kleene star. The algorithm should return a function that takes a string as input and returns \texttt{True} if the string matches the regular expression and \texttt{False} otherwise.
  \end{itemize}
